...
// ── Campus district zones — boundaries derived from real OSM building footprints ──
// Verified via Overpass API (2025). Key anchors:
//   Siebel Ctr CS:          bbox W=-88.2255 E=-88.2242 S=40.1137 N=40.1144
//   Grainger Eng Library:   bbox W=-88.2274 E=-88.2263 S=40.1123 N=40.1126
//   Engineering Hall:       bbox W=-88.2273 E=-88.2266 S=40.1107 N=40.1111
//   BIF (Gies Business):    bbox W=-88.2314 E=-88.2305 S=40.1032 N=40.1040
//   Turner Hall (ACES):     bbox W=-88.2245 E=-88.2239 S=40.1026 N=40.1033
//   Mumford Hall (ACES):    bbox W=-88.2261 E=-88.2258 S=40.1032 N=40.1040
//   Altgeld Hall (LAS):     bbox W=-88.2287 E=-88.2281 S=40.1091 N=40.1096
//   Noyes Laboratory (LAS): bbox W=-88.2265 E=-88.2257 S=40.1081 N=40.1088
//   Gregory Hall (Media):   bbox W=-88.2285 E=-88.2277 S=40.1055 N=40.1059
//   Krannert Ctr PAC (FAA): bbox W=-88.2235 E=-88.2220 S=40.1073 N=40.1088
//   Harding Band Bldg(FAA): bbox W=-88.2310 E=-88.2305 S=40.1046 N=40.1050
const campusZones: {
  id: string; name: string; nameEn: string; color: string;
  center: [number, number]; coords: [number, number][]
}[] = [
  {
    // Buildings: Siebel [W-88.2255,E-88.2242,S40.1137,N40.1144]
    //           + Grainger [W-88.2274,E-88.2263,S40.1123,N40.1126]
    //           + Engineering Hall [W-88.2273,E-88.2266,S40.1107,N40.1111]
    // Zone padded ~100 m around cluster
    id: 'eng', name: 'Engineering', nameEn: 'Grainger College', color: '#3b82f6',
    center: [-88.2261, 40.1128],
    coords: [
      [-88.2290, 40.1095],
      [-88.2278, 40.1092],
      [-88.2260, 40.1093],
  ...
  {
    // Building: BIF [W-88.2314,E-88.2305,S40.1032,N40.1040]
    // Zone extends to cover the Gies college district territory
    id: 'bus', name: 'Business', nameEn: 'Gies College', color: '#16a34a',
    center: [-88.2318, 40.1026],
    coords: [
      [-88.2352, 40.0994],
      [-88.2332, 40.0990],
      [-88.2312, 40.0992],
  ...
  {
    // Buildings: Turner Hall [W-88.2245,E-88.2239,S40.1026,N40.1033]
    //           + Mumford Hall [W-88.2261,E-88.2258,S40.1032,N40.1040]
    // Zone covers the ACES south-campus agricultural corridor
    id: 'agr', name: 'ACES', nameEn: 'Agricultural Sciences', color: '#f97316',
    center: [-88.2233, 40.1008],
    coords: [
      [-88.2276, 40.0976],
      [-88.2254, 40.0971],
      [-88.2228, 40.0972],
  ...
  {
    // Buildings: Altgeld Hall [W-88.2287,E-88.2281,S40.1091,N40.1096]
    //           + Noyes Laboratory [W-88.2265,E-88.2257,S40.1081,N40.1088]
    // Zone covers the LAS Wright-corridor cluster
    id: 'las', name: 'LAS', nameEn: 'Liberal Arts & Sciences', color: '#a855f7',
    center: [-88.2276, 40.1078],
    coords: [
      [-88.2314, 40.1052],
      [-88.2298, 40.1047],
      [-88.2278, 40.1046],
  ...
  {
    // Building: Gregory Hall [W-88.2285,E-88.2277,S40.1055,N40.1059]
    // Compact zone tightly wrapping the single building + immediate surroundings
  ...
  {
    // Buildings: Krannert Ctr PAC [W-88.2235,E-88.2220,S40.1073,N40.1088]
    //           + Harding Band Bldg [W-88.2310,E-88.2305,S40.1046,N40.1050]
    // FAA zone wraps Krannert (dominant landmark) with extension toward Harding
    id: 'art', name: 'FAA', nameEn: 'Fine & Applied Arts', color: '#0ea5e9',
    center: [-88.2228, 40.1081],
    coords: [
      [-88.2252, 40.1063],
      [-88.2236, 40.1060],
      [-88.2218, 40.1065],
